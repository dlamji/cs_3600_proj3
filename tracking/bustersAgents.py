...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        for ghostCounter in livingGhostPositionDistributions:
            maxp = 0
            maxpos = None
            for pos,possibility in ghostCounter.items(): 
                if (possibility>maxp):
                    maxp = possibility
                    maxpos = pos
            curdis = self.distancer.getDistance(pacmanPosition, maxpos)
        ...
        for move in legal:
            ...
            newdis = self.distancer.getDistance(successorPosition, closestpos)
            if(newdis < finaldis):
                finaldis = newdis
                finalmove = move
